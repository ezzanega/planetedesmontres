...
export class CartService {
  ...
  addCartProduct(payload: IProduct) {
    ...
    // Handle undefined quantity safely
    const payloadQuantity = payload.quantity ?? 0;

    if (payload.status === 'out-of-stock' || payloadQuantity === 0) {
    ...
    if (!isExist) {
      ...
      state.cart_products.forEach((item: IProduct) => {
        if (item.id === payload.id) {
          const quantity = item.quantity ?? 0;
          const orderQty = item.orderQuantity ?? 0;

          if (quantity >= orderQty + this.orderQuantity) {
            item.orderQuantity =
              this.orderQuantity !== 1
                ? orderQty + this.orderQuantity
                : orderQty + 1;
